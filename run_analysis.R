getwd()
"C:/Users/Vicki/Documents"
setwd("/Users/Vicki/Documents/R")
getwd()
"C:/Users/Vicki/Documents/R"
...
# Features filename given a dataset name.
features.file <- function(name) {
paste("X_", name, ".txt", sep = "")
}

# Activities filename given a dataset name.
activities.file <- function(name) {
paste("Y_", name, ".txt", sep = "")
}

# Subjects filename given a dataset name.
subjects.file <- function(name) {
paste("subject_", name, ".txt", sep = "")
}
 
# This returns an interim dataframe for a single dataset.
...
# Now read the subjects list.
p("  reading subjects...")
subjects.t <- read.table(subjects.name)
names(subjects.t) <- c("subject")
clean.data <- cbind(clean.data, subject = subjects.t$subject)

# Now return the clean data.
clean.data
}


# This performs the full analysis of both the test and train datasets.
# Write a clean dataset to disk.
run.analysis <- function(dir) {
p("Getting and Cleaning Data")
p("Author: Edmund Martin")
p("---")
p("Starting up.")
...
# Run the analysis.
run.analysis("/repos/Getting-and-Cleaning-Data/data")
[run_analysis.R] Getting and Cleaning Data 
[run_analysis.R] Author: Edmund Martin 
[run_analysis.R] --- 
[run_analysis.R] Starting up. 
[run_analysis.R] Preparing to run analysis. 
[run_analysis.R] Reading datasets. 
[run_analysis.R] Getting dataset: /repos/Getting-and-Cleaning-Data/data/test 
